...
def _strip_accents(text: str) -> str:
    """Return ``text`` lower-cased with combining accents removed.

    Python's standard library exposes ``unicodedata`` for accent stripping, but
    we purposely inline a compact solution to avoid importing the module in
    tight loops.  The function covers the limited subset of precomposed vowels
    that appear in the XML corpora bundled with the repository.
    """
    ...
    lowered = text.lower()
    for chars, replacement in basic:
        for char in chars:
...
class AnalysisResult:
    ...
    def as_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tokens": [
                {"text": token.text, "code": token.code} for token in self.tokens
            ],
...
class PhoneticTrailingSongModel:
    ...
    def phoneticize(self, text: str) -> List[PhoneticToken]:
        ...
        for match in _WORD_RE.finditer(text):
            word = match.group(0)
            tokens.append(PhoneticToken(text=word, code=self.phonetic_code(word)))
        return tokens
    ...
    def trailing_rhyme_key(self, line: str) -> str:
        ...
        if not words:
            return "".ljust(self.max_code_length, "0")
        return self.phonetic_code(words[-1].group(0))
    ...
    def line_title_sound(self, line: str) -> str:
        ...
        first = self.phonetic_code(words[0].group(0))
        last = self.phonetic_code(words[-1].group(0))
    ...
    def compare_texts(
        ...
        *,
        ...
        current = self.analyze(current_text)
        reference = self.analyze(reference_text)
...
def _cli(argv: Sequence[str]) -> int:
    ...
    model = PhoneticTrailingSongModel()
    analysis = model.analyze(contents)
    ...
    if args.compare_with:
        ...
        reference_analysis = model.analyze(reference_text)
...
def analyze_text(
    text: str, *, model: PhoneticTrailingSongModel | None = None
) -> AnalysisResult:
