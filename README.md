...
The planner now double-checks the coverage of every file and synthesises
"filler" chunks whenever a gap is detected so that no hole is left unattended.
The human-readable summary calls out any filled ranges and the JSON payload
records them under `hole_patches`, allowing downstream tooling to distinguish
between original and reconstructed spans.
